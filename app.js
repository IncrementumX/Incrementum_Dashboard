import { buildScoutModel } from "./src/scout/scout-v2.js";
...
function normalizeScoutState(_rawScout) {
  // Scout V2 has no persisted preferences. The legacy strategyLab / optionsCalc /
  // watchlist / customPairs blobs from older state versions are intentionally dropped.
  return {};
}
...
function setupScout() {
  // Scout V2 has no interactive controls — render is fully driven by the model.
  // Kept as a no-op so the call site at boot remains stable.
}

function buildScoutCacheKey(analytics) {
  const tickers = (analytics?.holdings || [])
    .map((h) => h.ticker)
    .filter((t) => t && t !== "CASH");
  return [...new Set(tickers)].sort().join(",");
}

function getScoutPortfolioTickers(analytics) {
  return [...new Set(
    (analytics?.holdings || [])
      .map((h) => h.ticker)
      .filter((t) => t && t !== "CASH")
...
async function refreshScoutModel({ tickers, cacheKey }) {
  syncUiState.scoutLoading = true;
  syncUiState.scoutError = null;
  try {
    const model = await buildScoutModel({ tickers });
    syncUiState.scoutModel = model;
    syncUiState.scoutCacheKey = cacheKey;
  } catch (error) {
    syncUiState.scoutError = error?.message || "Scout build failed";
  } finally {
    syncUiState.scoutLoading = false;
  }
  // Re-render with the freshly built model.
  if (elements.scoutRoot) renderScout(null, { holdings: tickers.map((t) => ({ ticker: t })) }, null);
}

function renderScout(_context, analytics, _summary) {
  if (!elements.scoutRoot) return;

  const tickers = getScoutPortfolioTickers(analytics);
  const cacheKey = buildScoutCacheKey(analytics);

  if (syncUiState.scoutCacheKey !== cacheKey && !syncUiState.scoutLoading) {
    void refreshScoutModel({ tickers, cacheKey });
  }

  if (syncUiState.scoutError) {
    elements.scoutRoot.innerHTML = renderScoutErrorPanel(syncUiState.scoutError);
    return;
  }

  if (!syncUiState.scoutModel) {
    elements.scoutRoot.innerHTML = renderScoutLoadingPanel();
    return;
  }

  const model = syncUiState.scoutModel;
  if (model.unavailable) {
    elements.scoutRoot.innerHTML = renderScoutUnavailablePanel(model.reason);
    return;
  }

  elements.scoutRoot.innerHTML = `
    ${renderScoutMetaBar(model.meta)}
    ${renderMacroPanel(model.macro, model.meta)}
    ${renderMomentumPanel(model.momentum, tickers)}
    ${renderCrossAssetPanel(model.crossAsset)}
    ${renderCorrelationPanel(model.correlation)}
    ${renderCalendarPanel(model.calendar)}
  `;
}

// --- Scout V2 sub-renderers ------------------------------------------------

function renderScoutLoadingPanel() {
  return `
    <section class="panel scout-empty-state scout-loading-panel">
      <div class="panel-header">
        <div>
          <h2>Loading Scout</h2>
          <p class="panel-subtitle">Fetching FRED macro + Yahoo prices...</p>
        </div>
...
function renderScoutErrorPanel(message) {
  return `
...
function renderScoutUnavailablePanel(reason) {
  return `
    <section class="panel scout-empty-state">
      <div class="panel-header">
        <div>
          <h2>Scout Unavailable</h2>
          <p class="panel-subtitle">${escapeHtml(reason || "Data source unreachable.")}</p>
        </div>
        <span class="status-pill status-pill--warning">Unavailable</span>
      </div>
    </section>
  `;
}

function renderScoutMetaBar(meta) {
  const stalePillClass =
    meta.stalenessLevel === "stale" ? "status-pill--error" :
    meta.stalenessLevel === "warn"  ? "status-pill--warning" :
    meta.stalenessLevel === "fresh" ? "status-pill--success" :
                                       "status-pill--muted";
  const stalePillLabel =
    meta.stalenessLevel === "stale" ? `Stale (${meta.agedDays}d)` :
    meta.stalenessLevel === "warn"  ? `Aging (${meta.agedDays}d)` :
    meta.stalenessLevel === "fresh" ? `Fresh (${meta.agedDays}d)` :
                                       "Unknown";
  return `
    <section class="panel scout-meta-bar">
      <div class="panel-header">
        <div>
          <h2>Macro / Signal Cockpit</h2>
          <p class="panel-subtitle">
            Source: <strong>${escapeHtml(meta.source)}</strong> ·
            Last data: <strong>${escapeHtml(meta.lastDataDate || "n/a")}</strong> ·
            Generated: <strong>${escapeHtml(meta.generatedAt || "n/a")}</strong>
          </p>
        </div>
        <span class="status-pill ${stalePillClass}">${escapeHtml(stalePillLabel)}</span>
      </div>
    </section>
  `;
}

function renderMacroPanel(macro, meta) {
  const items = (macro?.items || []).map((item) => {
    if (!item.available) {
      return `
        <article class="panel stat-panel">
          <p class="panel-label">${escapeHtml(item.label)}</p>
          <strong class="panel-value">Unavailable</strong>
          <p class="metric-footnote">Source: ${escapeHtml(item.source)}</p>
        </article>
      `;
    }
    const change = item.change20d;
    const changeStr = change >= 0 ? `+${change.toFixed(2)}` : change.toFixed(2);
    const changeClass = change >= 0 ? "is-positive" : "is-negative";
    const valueLabel = item.unit === "bps" ? `${item.value.toFixed(0)} bps` : `${item.value.toFixed(2)}%`;
    return `
      <article class="panel stat-panel">
        <p class="panel-label">${escapeHtml(item.label)}</p>
  ...
  const labels = (macro?.labels || []).map((l) => {
    const cls = l.sentiment === "negative" ? "scout-label--negative" :
                l.sentiment === "positive" ? "scout-label--positive" :
                                              "scout-label--neutral";
    return `<span class="scout-label ${cls}"><strong>${escapeHtml(l.label)}</strong> — ${escapeHtml(l.detail)}</span>`;
  }).join("");
  ...
  return `
    <section class="panel">
      <div class="panel-header">
        <div>
          <h2>1. Macro Regime</h2>
          <p class="panel-subtitle">FRED — 10Y nominal/real/breakeven, 2s10s curve, 5Y5Y forward inflation.</p>
        </div>
      </div>
      <div class="panel-grid panel-grid--summary">${items}</div>
      <div class="scout-label-row">${labels}</div>
    </section>
  `;
}
...
function renderMomentumPanel(momentum, tickers) {
  ...
  const rows = (momentum?.rows || []).map((row) => {
    ...
    const trendCls = row.trend === "Uptrend" ? "is-positive" :
                     row.trend === "Downtrend" ? "is-negative" :
                     "panel-subtitle";
    return `
      <tr>
        <td class="ticker-cell">${labelHtml(row)}</td>
        ...
        <td class="numeric-cell ${row.r20 >= 0 ? "is-positive" : "is-negative"}">${escapeHtml(fmtPct(row.r20))}</td>
        <td class="numeric-cell ${row.r60 >= 0 ? "is-positive" : "is-negative"}">${escapeHtml(fmtPct(row.r60))}</td>
        <td class="numeric-cell ${row.r180 >= 0 ? "is-positive" : "is-negative"}">${escapeHtml(fmtPct(row.r180))}</td>
        <td class="numeric-cell">${row.vol60 === null ? "—" : escapeHtml(`${(row.vol60 * 100).toFixed(1)}%`)}</td>
        <td class="numeric-cell is-negative">${row.drawdown180 === null ? "—" : escapeHtml(`${(row.drawdown180 * 100).toFixed(1)}%`)}</td>
        <td class="${trendCls}">${escapeHtml(row.trend)}</td>
      </tr>
    `;
  }).join("");

  const empty = tickers.length === 0
    ? `<p class="panel-subtitle">No portfolio holdings to evaluate. Import an IBKR statement first.</p>`
    : "";
        ...
        <table class="data-table">
          <thead>
            <tr>
              ...
              <th class="numeric-cell">1D</th>
              <th class="numeric-cell">5D</th>
              <th class="numeric-cell">20D</th>
              <th class="numeric-cell">60D</th>
              <th class="numeric-cell">180D</th>
              <th class="numeric-cell">Vol 60D (ann.)</th>
              <th class="numeric-cell">Max DD 180D</th>
              <th>Trend</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
...
function renderCrossAssetPanel(crossAsset) {
    ...
    if (!sig.available) {
      return `
        <article class="panel">
          ...
          <p class="panel-subtitle">Unavailable — ${escapeHtml(sig.reason || "")}</p>
        </article>
      `;
    }
    if (sig.legs) {
      const legHtml = sig.legs.map((leg) => `
        <div class="secondary-stat">
          <span>${escapeHtml(leg.label)}</span>
          <strong>${escapeHtml(typeof leg.value === "number" ? leg.value.toFixed(2) : "—")}</strong>
          <p class="secondary-stat-helper">20D Δ: ${escapeHtml((leg.change20d >= 0 ? "+" : "") + leg.change20d.toFixed(2))} · ${escapeHtml(leg.source)} · ${escapeHtml(leg.asOf)}</p>
        </div>
      `).join("");
      return `
        <article class="panel">
  ...
  return `
    <section class="panel">
      <div class="panel-header">
        <div>
          <h2>3. Cross-Asset Signals</h2>
          <p class="panel-subtitle">Gold/Silver, Gold vs Real Yields, Miners vs Metal, VIX context.</p>
        </div>
      </div>
      <div class="panel-grid panel-grid--analytics">${cards}</div>
    </section>
  `;
}

function renderCorrelationPanel(correlation) {
      ...
      <section class="panel">
        <div class="panel-header">
          <div>
            ...
            <p class="panel-subtitle">${escapeHtml(correlation?.reason || "Insufficient data.")}</p>
          </div>
        </div>
        ${(correlation?.tickersWithoutData || []).length > 0 ? `
          <p class="panel-subtitle">Without time series in scout-data.json: <strong>${escapeHtml(correlation.tickersWithoutData.join(", "))}</strong></p>
        ` : ""}
      </section>
    `;
  }

  const renderMatrix = (matrix, label, insufficient) => {
    if (insufficient || !matrix) {
      return `<p class="panel-subtitle">${escapeHtml(label)}: insufficient history.</p>`;
    }
    const head = `<tr><th></th>${correlation.tickers.map((t) => `<th>${escapeHtml(t)}</th>`).join("")}</tr>`;
    const body = correlation.tickers.map((t, i) => {
      const cells = correlation.tickers.map((_, j) => {
        const v = matrix[i][j];
        if (v === null) return `<td class="numeric-cell">—</td>`;
        const cls = Math.abs(v) >= 0.7 ? (v >= 0 ? "scout-corr--high-pos" : "scout-corr--high-neg") : "";
        return `<td class="numeric-cell ${cls}">${v.toFixed(2)}</td>`;
      }).join("");
    ...
    return `
      ...
      <div class="table-wrap">
        <table class="data-table scout-corr-table"><thead>${head}</thead><tbody>${body}</tbody></table>
      </div>
    `;
  };
...
function renderCalendarPanel(calendar) {
  const events = (calendar?.events || []).map((e) => `
    <tr>
      ...
      <td><span class="panel-subtitle">${escapeHtml(e.source)}${e.estimated ? " · estimated" : ""}</span></td>
    </tr>
  `).join("");
  ...
  return `
    <section class="panel">
      ...
      <div class="table-wrap">
        <table class="data-table">
          <thead>
            <tr><th>Date</th><th>Event</th><th>Category</th><th>Source</th></tr>
          </thead>
          <tbody>${events || `<tr><td colspan="4" class="panel-subtitle">No upcoming events.</td></tr>`}</tbody>
        </table>
      </div>
    </section>
  `;
